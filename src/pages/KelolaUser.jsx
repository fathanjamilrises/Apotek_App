...
function KelolaUser() {
  ...
  useEffect(() => {
    ...
    const selectedId = localStorage.setItem("selectedUserId", selectedUserId);
    if (userId) {
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="p-4 overflow-y-auto bg-slate-300 flex-1">
          ...
          <div className="mt-10 flex justify-end">
            <button
              className="btn btn-primary text-white text-md mr-5"
              onClick={toggleModal}
            >
              Tambah User
            </button>
          ...
          <div className="mt-10 mx-auto bg-white rounded-md w-full">
            <div className="overflow-x-auto">
              {isLoading ? (
                <div className="loadingSekeleton p-5">
                  <div className="skeleton h-5 w-full mb-5"></div>
                  <div className="loadingSekeletonRows flex">
                    <div className="skeleton h-12 w-[1090px] flex flex-column"></div>
                    <div className="skeleton h-12 w-16 ml-2 flex flex-column"></div>
                    <div className="skeleton h-12 w-16 ml-2 flex flex-column"></div>
                  </div>
                </div>
              ) : (
                  ...
                  <tbody>
                    {filteredUsers.map((user) => (
                      <tr key={user.id}>
                        ...
                        <td className="flex">
                          <button
                            className="btn btn-square btn-primary text-white text-md mr-2"
                          ...
                          <button
                            className="btn btn-square btn-error text-white text-md"
